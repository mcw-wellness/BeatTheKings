...
import { users, venues, cities, countries, sports, matches, activePlayers } from '@/db/schema'
...
vi.mock('@/lib/azure-storage', () => ({
  ...
  getDefaultAvatarSasUrl: (gender: string) => `https://test.blob.core.windows.net/default/${gender}`,
}))
...
import { canChallenge, cancelChallenge } from '@/lib/matches'
...
async function setupTestData() {
  const [country] = await testDb.insert(countries).values({ name: 'Austria', code: 'AT' }).returning()
  const [city] = await testDb.insert(cities).values({ name: 'Vienna', countryId: country.id }).returning()
  const [player1] = await testDb.insert(users).values({
    email: 'player1@test.com', name: 'Player One', cityId: city.id, hasCreatedAvatar: true, ageGroup: '18-30',
  }).returning()
  const [player2] = await testDb.insert(users).values({
    email: 'player2@test.com', name: 'Player Two', cityId: city.id, hasCreatedAvatar: true, ageGroup: '18-30',
  }).returning()
  const [venue] = await testDb.insert(venues).values({
    name: 'Test Court', cityId: city.id, isActive: true, latitude: 48.1962, longitude: 16.3551,
  }).returning()
  const [sport] = await testDb.insert(sports).values({
...
describe('1v1 Challenge E2E Flow', () => {
  ...
  describe('Full happy path: check-in → challenge → accept → start → upload → score → agree → complete', () => {
    it('should complete the entire 1v1 flow end-to-end', async () => {
      const { player1, player2, venue, sport } = await setupTestData()
      ...
      const checkIn2 = await checkInToVenue(testDb, player2.id, venue.id, 48.1961, 16.3550)
      ...
      const challengeReq = jsonRequest('http://localhost/api/challenges/1v1/request', {
        opponentId: player2.id, venueId: venue.id,
      })
      ...
      const acceptReq = jsonRequest(`http://localhost/api/challenges/1v1/${matchId}/respond`, { accept: true })
      ...
      mockGetSession.mockResolvedValue({ user: { id: player1.id } })
      const startReq = new Request(`http://localhost/api/challenges/1v1/${matchId}`, { method: 'POST' })
      ...
      const [matchInDb] = await testDb.select().from(matches).where(eq(matches.id, matchId)).limit(1)
      expect(matchInDb.status).toBe('in_progress')
      expect(matchInDb.videoUrl).toBeTruthy()

      // Step 7: Both players check out
      await checkOutFromVenue(testDb, player1.id)
      await checkOutFromVenue(testDb, player2.id)

      const status1 = await getUserCheckInStatus(testDb, player1.id)
  ...
  describe('Challenge cancel flow', () => {
    it('should allow challenger to cancel pending challenge', async () => {
      ...
      // Player 1 cancels
      capturedNotifications.length = 0
      const cancelReq = new Request(`http://localhost/api/challenges/1v1/${matchId}/cancel`, { method: 'POST' })
  ...
  describe('Stuck state and auto-expiry', () => {
    it('should block new challenge when one is already pending', async () => {
      ...
      await testDb.insert(matches).values({
        ...
        venueId: venue.id, sportId: (await testDb.select().from(sports).limit(1))[0].id,
        status: 'pending',
      })
    ...
    it('should auto-expire pending matches older than 30 minutes', async () => {
      ...
      await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
    ...
    it('should NOT auto-expire recent pending matches', async () => {
      ...
      await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
        status: 'pending', createdAt: recentDate,
      })
  ...
  describe('Decline flow', () => {
    it('should notify challenger when challenge is declined', async () => {
      ...
      capturedNotifications.length = 0
      mockGetSession.mockResolvedValue({ user: { id: player2.id } })
      const declineReq = jsonRequest(`http://localhost/api/challenges/1v1/${matchId}/respond`, { accept: false })
  ...
  describe('Upload permissions', () => {
    it('should allow upload when recordingBy is null (any participant)', async () => {
      ...
      const [match] = await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
        status: 'in_progress', recordingBy: null,
      }).returning()
    ...
    it('should reject upload from wrong player when recordingBy is set', async () => {
      ...
      const [match] = await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
        status: 'in_progress', recordingBy: player1.id,
      }).returning()
    ...
    it('should reject duplicate video upload', async () => {
      ...
      const [match] = await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
        status: 'in_progress', videoUrl: 'https://already-uploaded.mp4',
      }).returning()
  ...
  describe('Score submission and agreement', () => {
    it('should complete match when both players agree', async () => {
      ...
      const [match] = await testDb.insert(matches).values({
        player1Id: player1.id, player2Id: player2.id,
        venueId: venue.id, sportId: sport.id,
        status: 'completed', player1Score: 12, player2Score: 10,
        winnerId: player1.id, winnerXp: 100, winnerRp: 20, loserXp: 25,
      }).returning()

      // Player 1 agrees
      mockGetSession.mockResolvedValue({ user: { id: player1.id } })
      const agree1Req = jsonRequest(`http://localhost/api/challenges/1v1/${match.id}/agree`, { agree: true })
      ...
      capturedNotifications.length = 0
      mockGetSession.mockResolvedValue({ user: { id: player2.id } })
      const agree2Req = jsonRequest(`http://localhost/api/challenges/1v1/${match.id}/agree`, { agree: true })
    ...
    it('should handle dispute', async () => {
      ...
      mockGetSession.mockResolvedValue({ user: { id: player2.id } })
      const disputeReq = jsonRequest(`http://localhost/api/challenges/1v1/${match.id}/agree`, { agree: false })
      const disputeRes = await POST_AGREE(disputeReq, routeParams(match.id))
      expect(disputeRes.status).toBe(200)

      const [updatedMatch] = await testDb.select().from(matches).where(eq(matches.id, match.id)).limit(1)
  ...
  describe('Venue check-in and check-out', () => {
    it('should check in within distance and check out', async () => {
      ...
      const status = await getUserCheckInStatus(testDb, player1.id)
      expect(status.isCheckedIn).toBe(true)
      expect(status.venueId).toBe(venue.id)

      // Check out
      const checkOut = await checkOutFromVenue(testDb, player1.id)
      expect(checkOut.success).toBe(true)

      const statusAfter = await getUserCheckInStatus(testDb, player1.id)
  ...
  describe('SSE notification events', () => {
    it('should send correct notification types for each action', async () => {
      ...
      expect(capturedNotifications.some(n =>
        n.userId === player2.id && n.event.type === 'challenge-received'
      )).toBe(true)
      ...
      expect(capturedNotifications.some(n =>
        n.userId === player1.id && n.event.type === 'challenge-accepted'
      )).toBe(true)
    })
  })
})
