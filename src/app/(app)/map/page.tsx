...
function MapPageContent(): JSX.Element {
  ...
  const { latitude, longitude, loading: geoLoading, error: geoError, permission, requestPermission } = useLocation()
  ...
  const fetchVenueDetails = useCallback(async (venueId: string): Promise<void> => {
  ...
  useEffect(() => { if (!geoLoading) fetchVenues() }, [geoLoading, fetchVenues])
  useEffect(() => { if (venueIdFromUrl && !isLoading) fetchVenueDetails(venueIdFromUrl) }, [venueIdFromUrl, isLoading, fetchVenueDetails])
